...
public final class Logger {
	...
	public static func log<T: CustomStringConvertible>(_ message: T, type: OSLogType = .default, category: Category = .app, accessLevel: AccessLevel = .public,
						   options: [Option] = Option.allCases,
		...
		switch accessLevel {
		case .public:
			os_log("[%{public}s][%{public}s] \n> %{public}s", log: category.osLog, type: type, extraInfo, "\(type)", message.description)
		case .private:
			os_log("[%{public}s][%{public}s] \n> %{private}s", log: category.osLog, type: type, extraInfo, "\(type)", message.description)
		}
	}
